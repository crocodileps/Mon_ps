                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                  {[
                    ...
                    return (
                      <div
                        key={market.key}
                        onClick={() => setSelectedMarket({ ...market, score, rec, value, kelly, odds: market.data?.odds || 0, matchName: `${analysis.home_team} vs ${analysis.away_team}` })}
                        className={[
                      ...
                      >
                        <div className="text-xs text-slate-400 flex items-center justify-center gap-1">
                          {market.label}
                          {value === '💎 DIAMOND VALUE' && <span title="Diamond Value">💎</span>}
                          {value === '🔥 STRONG VALUE' && <span title="Strong Value">🔥</span>}
                        </div>
                        ...
                        {kelly > 0 && <div className="text-xs text-cyan-400 font-medium">K:{kelly.toFixed(1)}%</div>}
                      </div>
                    );
                  })}
                </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center p-3 bg-slate-900/50 rounded-lg">
                  <span className="text-slate-400">Cote</span>
                  <span className="text-yellow-400 font-semibold">{selectedMarket.odds > 0 ? selectedMarket.odds.toFixed(2) : "N/A"}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-slate-900/50 rounded-lg">
                  <span className="text-slate-400">Edge</span>
                  <span className={["font-semibold", selectedMarket.odds > 0 && selectedMarket.score > 0 ? (selectedMarket.score/100 - 1/selectedMarket.odds) > 0 ? "text-emerald-400" : "text-red-400" : "text-slate-500"].join(" ")}>
                    {selectedMarket.odds > 0 && selectedMarket.score > 0 ? ((selectedMarket.score/100 - 1/selectedMarket.odds) * 100).toFixed(1) + "%" : "N/A"}
                  </span>
                </div>
              </div>
