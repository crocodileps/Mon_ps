...
export default function CombosPage() {
  ...
  const addSuggestionToBuilder = (suggestion: ComboSuggestion) => {
    const newPicks = suggestion.picks.map((p, idx) => ({
      ...
      odds: p.odds,
      score: p.score,
      win_rate: suggestion.combined_probability / suggestion.picks.length
    }));
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {suggestion.picks.map((pick, pidx) => (
                          <div key={pidx} className="flex items-center gap-2 bg-slate-700/50 rounded-lg px-3 py-2">
                            ...
                            <span className="text-cyan-400 font-bold">@{(pick.odds ?? 0).toFixed(2)}</span>
                      ...
                      <div className="grid grid-cols-4 gap-4 p-3 bg-slate-900/50 rounded-lg">
                        <div>
                          ...
                          <p className="text-xl font-bold text-cyan-400">{(suggestion.combined_odds ?? 0).toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-slate-500 text-xs">Win Rate</p>
                          <p className="text-xl font-bold text-emerald-400">{(suggestion.combined_probability ?? 0).toFixed(1)}%</p>
                        </div>
                        <div>
                          ...
                          <p className={`text-xl font-bold ${suggestion.correlation_score < 0.4 ? 'text-emerald-400' : suggestion.correlation_score < 0.6 ? 'text-amber-400' : 'text-red-400'}`}>
                            {(((suggestion.correlation_score ?? 0) * 100)).toFixed(0)}%
                          </p>
                        </div>
                        <div>
                          <p className="text-slate-500 text-xs">Gain (10€)</p>
                          <p className="text-xl font-bold text-purple-400">{((10 * (suggestion.combined_odds ?? 0))).toFixed(2)}€</p>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-gradient-to-br from-purple-900/30 to-cyan-900/30 rounded-lg space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-cyan-400 font-bold">{(comboStats.totalOdds ?? 0).toFixed(2)}</span>
                    </div>
                    <hr className="border-slate-700" />
                    <div className="flex justify-between">
                      <span className="text-slate-300 font-medium">Gain Potentiel</span>
                      <span className="text-2xl font-bold text-yellow-400">{(comboStats.potentialWin ?? 0).toFixed(2)}€</span>
                    </div>
                  </div>
                  ...
                  {history.map((combo, idx) => (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        {(typeof combo.selections === 'string' ? JSON.parse(combo.selections).picks : combo.selections.picks || []).map((sel: any, sidx: number) => (
