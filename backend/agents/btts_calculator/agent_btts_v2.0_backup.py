...
class AgentBTTSCalculatorV2:
        ...
        try:
            with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if row:
                    ...
                    # Stats BTTS
                    profile.home_btts_rate = float(row.get('home_btts_rate') or 50) / 100
                    profile.away_btts_rate = float(row.get('away_btts_rate') or 50) / 100
                    profile.btts_tendency = float(row.get('btts_tendency') or 50) / 100
                    
                    # Stats offensives
                    profile.home_goals_scored_avg = float(row.get('home_goals_scored_avg') or 1.3)
                    profile.away_goals_scored_avg = float(row.get('away_goals_scored_avg') or 1.0)
                    profile.home_failed_to_score_rate = float(row.get('home_failed_to_score_rate') or 20) / 100
                    profile.away_failed_to_score_rate = float(row.get('away_failed_to_score_rate') or 30) / 100
                    
                    # Stats défensives
                    profile.home_goals_conceded_avg = float(row.get('home_goals_conceded_avg') or 1.0)
                    profile.away_goals_conceded_avg = float(row.get('away_goals_conceded_avg') or 1.4)
                    profile.home_clean_sheet_rate = float(row.get('home_clean_sheet_rate') or 30) / 100
                    profile.away_clean_sheet_rate = float(row.get('away_clean_sheet_rate') or 20) / 100
                    
                    # xG
                    profile.xg_for_avg = float(row.get('xg_for_avg') or 0)
                    profile.xg_against_avg = float(row.get('xg_against_avg') or 0)
                    profile.xg_difference = float(row.get('xg_difference') or 0)
                    profile.overperformance_goals = float(row.get('overperformance_goals') or 0)
                    
                    # Timing
                    profile.first_half_goals_pct = float(row.get('first_half_goals_pct') or 45) / 100
                    profile.second_half_goals_pct = float(row.get('second_half_goals_pct') or 55) / 100
                    profile.late_goals_pct = float(row.get('late_goals_pct') or 15) / 100
                    profile.conceded_late_pct = float(row.get('conceded_late_pct') or 15) / 100
                    profile.first_scorer_rate = float(row.get('first_scorer_rate') or 50) / 100
                    
                    # Style
                    profile.current_style = row.get('current_style') or 'balanced'
                    profile.current_pressing = row.get('current_pressing') or 'medium'
                    profile.style_score = float(row.get('current_style_score') or 50)
                    
                    # Contexte
                    profile.after_europe = float(row.get('after_europe') or 0)
                    profile.congested_schedule = float(row.get('congested_schedule') or 0)
                    profile.vs_top_teams = float(row.get('vs_top_teams') or 0)
                    profile.vs_bottom_teams = float(row.get('vs_bottom_teams') or 0)
                    
                    # Coach
                    profile.coach_style = row.get('coach_style') or ''
                    profile.coach_tenure_months = float(row.get('coach_tenure_months') or 0)
                    
                    # Over rates
                    profile.home_over25_rate = float(row.get('home_over25_rate') or 50) / 100
                    profile.away_over25_rate = float(row.get('away_over25_rate') or 50) / 100
                    profile.home_over15_rate = float(row.get('home_over15_rate') or 70) / 100
                    profile.away_over15_rate = float(row.get('away_over15_rate') or 65) / 100
                    
                    # Métadonnées
                    profile.matches_analyzed = int(row.get('matches_analyzed') or 0)
                    profile.confidence = float(row.get('confidence_overall') or 50)
                    profile.data_quality_score = float(row.get('data_quality_score') or 50)
    ...
    def calculate_btts_probability(self,
        ...
        if home_profile.xg_for_avg > 0:
            home_xg = home_xg * 0.7 + home_profile.xg_for_avg * 0.3
        if away_profile.xg_for_avg > 0:
            away_xg = away_xg * 0.7 + away_profile.xg_for_avg * 0.3
        ...
        style_adj = self._calculate_style_adjustment(home_profile, away_profile)
        ...
        p_home_scores = 1 - (home_profile.home_failed_to_score_rate * 0.7 + 
                            away_profile.away_clean_sheet_rate * 0.3)
        p_away_scores = 1 - (away_profile.away_failed_to_score_rate * 0.7 + 
                            home_profile.home_clean_sheet_rate * 0.3)
