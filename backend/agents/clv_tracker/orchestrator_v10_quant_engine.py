...
class OrchestratorV10Quant:
    ...
    def _prefetch_context(self, home_team: str, away_team: str,
        ...
        context = {
            ...
            'home_scorers': self._get_scorer_intelligence(home_team, 'home'),
            'away_scorers': self._get_scorer_intelligence(away_team, 'away'),
    ...
    def _calculate_mc_score(self, pick: QuantPick, mc_result: MonteCarloResult, context: Dict = None) -> int:
        ...
        if context:
            ...
            # xG INTELLIGENCE BLEND - Utiliser tendances Understat
            home_xg_intel = context.get('home_xg')
            away_xg_intel = context.get('away_xg')
            
            if home_xg_intel and away_xg_intel:
                xg_adjustment = 0.0
                
                if 'btts' in market:
                    # Moyenne des taux BTTS xG des deux equipes
                    home_btts_xg = home_xg_intel.get('btts_xg_rate', 50) / 100
                    away_btts_xg = away_xg_intel.get('btts_xg_rate', 50) / 100
                    avg_btts_xg = (home_btts_xg + away_btts_xg) / 2
                    
                    if 'yes' in market:
                        xg_adjustment = (avg_btts_xg - 0.5) * 0.15  # +/-7.5% max
                    else:
                        xg_adjustment = (0.5 - avg_btts_xg) * 0.15
                        
                elif 'over_25' in market or 'under_25' in market:
                    # Moyenne des taux Over2.5 xG
                    home_over_xg = home_xg_intel.get('over25_xg_rate', 50) / 100
                    away_over_xg = away_xg_intel.get('over25_xg_rate', 50) / 100
                    avg_over_xg = (home_over_xg + away_over_xg) / 2
                    
                    if 'over' in market:
                        xg_adjustment = (avg_over_xg - 0.5) * 0.15
                    else:
                        xg_adjustment = (0.5 - avg_over_xg) * 0.15
                
                mc_prob += xg_adjustment
                mc_prob = max(0.05, min(0.95, mc_prob))
