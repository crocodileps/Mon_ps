...
Le moteur le plus avancé avec:
- Machine Learning (Random Forest + Gradient Boosting)
- Backtesting automatique avec ROI/CLV
- Auto-ajustement des pondérations
- Ensemble de modèles
...
try:
    ...
    ML_AVAILABLE = False
    logging.warning("scikit-learn non disponible - ML désactivé")
...
class MLPredictor:
    """
    Module Machine Learning pour prédictions
    - Random Forest
    - Gradient Boosting
    - Ensemble voting
    - Calibration des probabilités
    ...
    def _create_models(self, target: str):
        """Crée les modèles pour un target (btts ou over25)"""
    ...
    def predict(self, features: FeatureSet, target: str) -> Dict:
        ...
        Returns:
            Probabilité moyenne + par modèle
        """
        if not self.is_trained or target not in self.models:
            return {'probability': None, 'error': 'Modèle non entraîné'}
...
class Backtester:
    ...
    def run_backtest(self, strategy: str = 'btts', 
        ...
        return BacktestResult(
            ...
            sharpe_ratio=sharpe,
            clv=0,  # À calculer avec les closing lines
...
class AutoCalibrator:
    """
    Auto-ajustement des pondérations basé sur performance historique
    """
    ...
    def calibrate(self, backtest_results: List[Dict]) -> Dict:
        """
        Calibre les poids basé sur les résultats de backtest
        
        Principe: augmenter le poids des facteurs qui ont bien prédit,
        diminuer ceux qui ont mal prédit
        """
        if not backtest_results:
            return self.default_weights.copy()
        
        # Analyser quels facteurs corrèlent avec le succès
        ...
        for result in backtest_results:
            ...
            for factor, value in factors.items():
                if factor in factor_success:
                    # Si le facteur était élevé et on a gagné, c'est positif
                    # Si le facteur était élevé et on a perdu, c'est négatif
                    correlation = value / 100 if won else -value / 100
                    factor_success[factor].append(correlation)
        
        # Calculer nouveaux poids basés sur corrélation moyenne
        new_weights = {}
        total = 0
        
        for factor, correlations in factor_success.items():
            if correlations:
                # Score = moyenne des corrélations + baseline
...
class PredictionEngineUltimate:
    """
    Moteur V4 ULTIMATE - Le plus avancé
    ...
    def __init__(self):
        ...
        # Charger modèles ML si disponibles
        self.ml.load()
        
        # Charger poids calibrés
    ...
    def get_team_data(self, team_name: str) -> Optional[Dict]:
        """Récupère données complètes d'une équipe"""
        conn = self._get_conn()
        cur = conn.cursor(cursor_factory=RealDictCursor)
    ...
    def train_ml_models(self) -> Dict:
        ...
        if len(matches) < 50:
            return {'error': f'Pas assez de données ({len(matches)} matchs, minimum 50)'}
    ...
    def calculate_score(self, features: FeatureSet, target: str, poisson_prob: float, h2h: Dict) -> Dict:
        """
        Calcule le score avec tous les modèles combinés
        """
        scores = {}
        ...
        # 2. Score statistique (stats globales + spécifiques + tendances)
        if target == 'btts':
            ...
            stats_specific = stats_global  # Utiliser global si spécifique non dispo
    ...
    def calculate_confidence(self, features: FeatureSet, h2h: Dict, 
        ...
        """Calcule confidence multi-critères"""
        score = 0
    ...
    def get_recommendation(self, score: float, confidence: Confidence) -> Recommendation:
        """Recommandation basée sur score et confidence"""
    ...
    def predict_match(self, home_team: str, away_team: str,
        ...
        """
        Prédiction ULTIMATE avec tous les modèles
        """
        # Extract features
    ...
    def recalibrate_weights(self) -> Dict:
        """Recalibre les poids basé sur performance"""
        # Récupérer résultats récents
        conn = self._get_conn()
        cur = conn.cursor(cursor_factory=RealDictCursor)
