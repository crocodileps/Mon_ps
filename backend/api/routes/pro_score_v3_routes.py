...
- POST /api/pro/check-correlation - Vérifier corrélation entre picks
- POST /api/pro/build-combo - Construire un combo optimisé
...
async def get_match_score(
    ...
    Utilise toutes les données FERRARI Intelligence:
...
async def calculate_score(request: MatchScoreRequest):
    """
    Calcule le Score PRO V3.1 avec tous les paramètres
...
async def get_matches_today_enriched(
    ...
    """
    Récupère tous les matchs du jour avec Score PRO enrichi
    
    Combine:
    - Données Full Gain 2.0
    - Sweet Spots CLV
    ...
    try:
        ...
        # Récupérer matchs du jour depuis full_gain_matches ou odds_history
...
async def get_elite_picks():
    """
    Récupère uniquement les picks ELITE (score >= 90)
    
    Ce sont les meilleures opportunités du jour
    """
    result = await get_matches_today_enriched(min_score=90, tier="ELITE")
    
    return {
        "tier": "ELITE",
        "description": "Picks avec score PRO >= 90. Conditions optimales réunies.",
...
async def get_diamond_picks():
    """
    Récupère les picks DIAMOND (score 80-89)
    """
    ...
    return {
        "tiers": ["ELITE", "DIAMOND"],
        "description": "Picks avec score PRO >= 80. Excellentes opportunités.",
...
async def get_value_alerts():
    """
    Récupère les alertes VALUE du jour
    ...
    try:
        ...
        # Analyser volatilité de chaque pattern
        alerts = []
        for p in patterns:
        ...
        return {
            ...
            "warning": "Les alertes volatiles nécessitent un bankroll important et une tolérance aux pertes consécutives"
        }
...
async def get_divergence_alerts():
    ...
    return {
        "date": str(datetime.now().date()),
        "description": "Matchs où les sources ne sont pas d'accord. Analyse manuelle recommandée.",
...
async def check_correlation(request: CorrelationRequest):
    """
    Vérifie la corrélation entre plusieurs picks
...
async def build_combo(request: ComboRequest):
    """
    Construit un combo optimisé avec faible corrélation
...
async def get_team_intelligence(team_name: str):
    """
    Récupère l'intelligence complète d'une équipe
    
    83 colonnes de données!
    """
    calculator = ProScoreV3Calculator()
...
async def get_scorer_intelligence(player_name: str):
    """
    Récupère l'intelligence complète d'un buteur
    
    153 colonnes de données!
    """
    conn = db_pool.get_connection()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
...
async def get_profitable_patterns(
    ...
    """
    Récupère les patterns profitables avec analyse de volatilité
    """
    conn = db_pool.get_connection()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
