...
async def analyze_conseil_ultim(match_id: str):
    """
    Agent Conseil Ultim 2.0 - Analyse TOUS les outcomes et recommande le meilleur
    ...
    try:
        ...
        # Récupérer les cotes par outcome
        cur.execute("""
            SELECT 
                outcome,
                MIN(odds_value) as min_cote,
                MAX(odds_value) as max_cote,
                AVG(odds_value) as avg_cote,
                COUNT(*) as nb_bookmakers,
                home_team,
                away_team,
                sport
            FROM odds
        ...
        patron_score = patron_analysis.get("score_v2", {}).get("score", 50)
        patron_outcome = patron_analysis.get("score_v2", {}).get("predicted_outcome", "home")
        
        # Analyser chaque outcome
        recommendations = []
        
        for outcome in outcomes_data:
            outcome_type = outcome['outcome']
            avg_cote = float(outcome['avg_cote'])
            nb_books = outcome['nb_bookmakers']
            ...
            notre_proba = max(5, min(95, notre_proba))
            
            # Edge réel
            edge_reel = notre_proba - proba_implicite
            
            # Score liquidité
            score_liquidite = min(100, (nb_books / 20) * 100)
            
            # Score composite final
            score_final = (
                (notre_proba * 0.4) +
                ((edge_reel + 20) * 0.3 * 2.5) +
                (patron_score * 0.2) +
                (score_liquidite * 0.1)
            )
            ...
            # Gain attendu
            gain_attendu = (edge_reel / 100) * avg_cote if edge_reel > 0 else edge_reel / 100
            
            # Risque
            if score_final >= 75:
                risque, risque_emoji = "FAIBLE", "✅"
            elif score_final >= 60:
                risque, risque_emoji = "MODÉRÉ", "⚠️"
            elif score_final >= 40:
                risque, risque_emoji = "ÉLEVÉ", "🔶"
            else:
                risque, risque_emoji = "TRÈS ÉLEVÉ", "❌"
            
            # Conseil
            if score_final >= 70 and edge_reel > 0:
                conseil = "RECOMMANDÉ"
            elif score_final >= 50:
            ...
            if outcome_type == 'home':
                label = f"🏠 {outcome['home_team'].split()[0].upper()}"
            elif outcome_type == 'away':
                label = f"✈️ {outcome['away_team'].split()[0].upper()}"
            else:
                label = "⚖️ MATCH NUL"
            ...
            recommendations.append({
                ...
                "label": label,
                "cote_moyenne": round(avg_cote, 2),
                "proba_marche": round(proba_implicite, 1),
                "notre_proba": round(notre_proba, 1),
                "edge_reel": round(edge_reel, 1),
                "score_final": round(score_final, 1),
                "gain_attendu": round(gain_attendu * 100, 1),
                "risque": risque,
                "risque_emoji": risque_emoji,
                "conseil": conseil,
                "nb_bookmakers": nb_books
            })
        ...
        # Confiance
        confiance = best["score_final"]
        if confiance >= 80:
            confiance_label = "TRÈS HAUTE"
        elif confiance >= 70:
            confiance_label = "HAUTE"
        elif confiance >= 50:
        ...
        response_data = {
            ...
            "match_info": {
                ...
                "away_team": outcomes_data[0]['away_team'],
                "sport": outcomes_data[0]['sport']
            },
            "recommandation_finale": best,
            "toutes_options": recommendations,
            "confiance_globale": {
                "score": round(confiance, 1),
                "label": confiance_label
            },
            "agent_patron": {
                "score": patron_score,
...
async def batch_conseil_ultim(match_ids: list[str]):
    """
    Retourne les recommandations finales pour plusieurs matchs
    ...
    try:
        ...
        for match_id in match_ids[:50]:  # Limiter à 50
            try:
                # Récupérer outcomes
                cur.execute("""
                    SELECT 
                    ...
                    FROM odds
                    WHERE match_id = %s
                    GROUP BY outcome, home_team, away_team
                """, (match_id,))
                ...
                # Analyser chaque outcome et trouver le meilleur
                best_score = 0
                best_outcome = None
                
                for opp in outcomes:
                    outcome_type = opp['outcome']
                    avg_cote = float(opp['avg_cote'])
                    nb_books = opp['nb_bookmakers']
                    ...
                    if score_final > best_score:
                        ...
                        best_outcome = {
                            ...
                            'home': outcomes[0]['home_team'],
                            'away': outcomes[0]['away_team']
                        }
                
                if best_outcome:
                    # Label
                    if best_outcome['outcome'] == 'home':
                        label = f"🏠 {best_outcome['home'].split()[0].upper()}"
                    elif best_outcome['outcome'] == 'away':
                        label = f"✈️ {best_outcome['away'].split()[0].upper()}"
                    else:
                        label = "⚖️ NUL"
