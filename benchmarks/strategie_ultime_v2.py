...
"""
╔═══════════════════════════════════════════════════════════════════════════════╗
║     🏆 STRATÉGIE ULTIME V2 - FUSION DE TOUTES LES DÉCOUVERTES                 ║
║                                                                               ║
║  Combine:                                                                     ║
║  • STRATÉGIE ULTIME V1 (+337.1u, 76.6% WR, +50% ROI)                         ║
║  • Monte Carlo V2 comme FILTRE de confirmation                               ║
║  • QUANT_BEST_MARKET pour équipes spécifiques                                ║
║                                                                               ║
║  Objectif: Maximiser ROI ET P&L en filtrant avec MC                          ║
╚═══════════════════════════════════════════════════════════════════════════════╝
"""
...
# Équipes ELITE (100% WR historique)
...
# Équipes QUANT (best_market spécifique performant)
QUANT_SPECIALISTS = ['Lazio', 'Athletic Club', 'Real Sociedad', 'Angers', 'Rayo Vallecano', 'Paris FC']

# Équipes CHAOS (TOTAL_CHAOS performant)
...
class StrategieUltimeV2:
    """Stratégie Ultime V2 avec Monte Carlo comme filtre"""
    ...
    def __init__(self):
        ...
        self.results = {
            ...
            'CONVERGENCE_MC': {'bets': 0, 'wins': 0, 'profit': 0},
    ...
    def monte_carlo_simulate(self, home_data: Dict, away_data: Dict, n_sims: int = 3000) -> MCResult:
    ...
    def get_decision(self, home_name: str, away_name: str) -> Optional[Decision]:
        """Décision avec filtre Monte Carlo"""
        ...
        for team_name in [home_name, away_name]:
            if self.is_in_list(team_name, QUANT_SPECIALISTS):
                ...
                if best_market == 'over_25' and mc.over_25_prob >= 0.55:
                    mc_confirms = True
                elif best_market == 'under_25' and mc.under_25_prob >= 0.50 and mc.expected_goals <= 2.5:
                    mc_confirms = True
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # RÈGLE 5: CONVERGENCE STANDARD + MC >= 60%
        # Stake: 2.0u
        # ═══════════════════════════════════════════════════════════════════════
        if convergence_over and mc.over_25_prob >= 0.60:
            return Decision(
                ...
                market='over_25',
                stake=2.0,
                category='CONVERGENCE_MC',
                mc_prob=mc.over_25_prob,
                expected_goals=mc.expected_goals,
                reason=f"CONVERGENCE_OVER + MC {mc.over_25_prob*100:.0f}%"
            )
            
        if convergence_under and mc.under_25_prob >= 0.52 and mc.expected_goals <= 2.4:
            return Decision(
                team=f"{home_name} vs {away_name}",
                market='under_25',
                stake=2.0,
                category='CONVERGENCE_MC',
                mc_prob=mc.under_25_prob,
                expected_goals=mc.expected_goals,
                reason=f"CONVERGENCE_UNDER + MC {mc.under_25_prob*100:.0f}%"
            )
            
        # ═══════════════════════════════════════════════════════════════════════
        # RÈGLE 6: MC TRÈS FORT (>= 70%) sans convergence mais pas de clash
        # Stake: 1.5u (prudent)
        # ═══════════════════════════════════════════════════════════════════════
        style_clash = (home_tendency == "OVER" and away_tendency == "UNDER") or \
                     (home_tendency == "UNDER" and away_tendency == "OVER")
                     
        if not style_clash and mc.over_25_prob >= 0.70:
            return Decision(
                team=f"{home_name} vs {away_name}",
                market='over_25',
                stake=1.5,
                ...
                mc_prob=mc.over_25_prob,
                expected_goals=mc.expected_goals,
                reason=f"MC PURE {mc.over_25_prob*100:.0f}% (no clash)"
            )
    ...
    def run_backtest(self):
        print("\n" + "="*100)
        print("🏆 BACKTEST STRATÉGIE ULTIME V2 (MC FILTER)")
        print("="*100)
    ...
    def print_results(self):
        print("\n" + "="*100)
        print("📊 RÉSULTATS STRATÉGIE ULTIME V2")
        print("="*100)
        ...
        print(f"{'='*100}")
        print(f"\n{'Catégorie':<20} {'Paris':<10} {'Wins':<10} {'WR':<12} {'P&L':<12}")
        print("-"*65)
        
        for cat in ['ELITE_MC', 'GOLD_MC', 'QUANT_MC', 'CHAOS_MC', 'CONVERGENCE_MC', 'STANDARD']:
            r = self.results[cat]
            if r['bets'] > 0:
                wr = (r['wins'] / r['bets']) * 100
                emoji = "💎" if r['profit'] > 40 else "✅" if r['profit'] > 0 else "❌"
                print(f"{emoji} {cat:<18} {r['bets']:<10} {r['wins']:<10} {wr:.1f}%{'':<7} {r['profit']:+.1f}u")
        ...
        print(f"\n{'#':<3} {'Équipe':<35} {'Cat':<15} {'Paris':<8} {'WR':<10} {'P&L':<10}")
        print("-"*85)
        
        for i, t in enumerate(team_list[:20], 1):
            emoji = "💎" if t['profit'] >= 15 else "✅" if t['profit'] > 0 else "❌"
            print(f"{emoji}{i:<2} {t['team'][:34]:<35} {t['cat'][:14]:<15} {t['bets']:<8} {t['wr']:.1f}%{'':<5} {t['profit']:+.1f}u")
        ...
        if all_r['bets'] > 0:
            ...
            print("-"*80)
            print(f"{'🏆 STRATÉGIE ULTIME V2':<30} {all_r['bets']:<10} {wr:.1f}%{'':<7} {roi:+.1f}%{'':<8} {all_r['profit']:+.1f}u")
            print(f"{'🥈 STRATÉGIE ULTIME V1':<30} {'244':<10} {'76.6%':<12} {'+50.0%':<14} {'+337.1u':<12}")
            print(f"{'🥉 MC V2 (corrigé)':<30} {'178':<10} {'73.6%':<12} {'+35.6%':<14} {'+168.6u':<12}")
            print(f"{'CONVERGENCE_OVER':<30} {'366':<10} {'73.8%':<12} {'+36.5%':<14} {'+400.5u':<12}")
            print(f"{'Adaptive Engine':<30} {'351':<10} {'69.8%':<12} {'+48.3%':<14} {'+372.8u':<12}")
            print(f"{'V13 Reference':<30} {'204':<10} {'76.5%':<12} {'+53.2%':<14} {'+210.7u':<12}")
            
        # Save
        with open('strategie_ultime_v2_results.json', 'w') as f:
            json.dump({
...
def main():
    print("""
╔═══════════════════════════════════════════════════════════════════════════════╗
║     🏆 STRATÉGIE ULTIME V2 - FUSION COMPLÈTE                                  ║
║                                                                               ║
║  Combine:                                                                     ║
║  • STRATÉGIE ULTIME V1 (équipes ELITE/GOLD/QUANT/CHAOS)                      ║
║  • Monte Carlo V2 comme FILTRE de confirmation                               ║
║  • Seuils MC calibrés par catégorie                                          ║
║                                                                               ║
║  Objectif: Battre V1 en qualité tout en gardant le volume                    ║
╚═══════════════════════════════════════════════════════════════════════════════╝
    """)
    
    strategy = StrategieUltimeV2()
    strategy.connect()
    strategy.load_data()
